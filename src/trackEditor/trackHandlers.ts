...
import { addKeyframesToSelection, clearTimelineSelection, removeKeyframesFromSelection, toggleKeyframeSelection } from "~/timeline/timelineSelectionSlice";
import {
        ...
        mouseDownMoveAction(e, {
            ...
            beforeMove: (params) => {
                ...
                if (additiveSelection) {
                    ...
                    params.dispatch([
                        ...timelineIds.map(id => clearTimelineSelection({ timelineId: id })),
                        toggleKeyframeSelection({ timelineId: timeline.id, keyframeId: keyframe.id }),
                        clearCompositionSelection(compositionId),
                        addLayerToSelection(compositionId, property.layerId),
                        addPropertyToSelection(compositionId, property.id)
                    ]);
                }
            },
            mouseMove: (params, { moveVector }) => {
                ...
                params.dispatch(
                    timelineIds.map(id => setIndexAndValueShift({ timelineId: id, indexShift: moveX, valueShift: 0 }))
                );
            },
            mouseUp: (params, hasMoved) => {
                ...
                const actions = timelineIds.reduce<any[]>((arr, id) => {
                    arr.push(
            ...
            beforeMove: (params) => {
                ...
                for (const { timelineId, keyframeIndex } of klist) {
                    ...
                    if (additiveSelection) {
                        op.add(
                            anyNotSelected
                                ? addKeyframesToSelection({ timelineId: timeline.id, keyframeIds: [keyframe.id] })
                                : removeKeyframesFromSelection({ timelineId: timeline.id, keyframeIds: [keyframe.id] }),
                        );
    ...
    layerMouseDown: (
        ...
        mouseDownMoveAction(e, {
            ...
            beforeMove: (params) => {
                ...
                if (additiveSelection) {
                    params.dispatch(
                        toggleLayerSelection(composition.id, layerId),
                    );
                    ...
                    if (compositionSelection.layers[layerId]) {
                        ...
                        params.dispatch(
                            layerTimelineIds.map((id) => clearTimelineSelection({ timelineId: id })),
                        );
                    }
                } else if (!compositionSelection.layers[layerId]) {
                    // If the current layer is not selected, we clear the selections of all timelines
                    // in the composition
                    params.dispatch(timelineIds.map((id) => clearTimelineSelection({ timelineId: id })));
                    params.dispatch(
                        toggleLayerSelection(composition.id, layerId),
                    );
                    params.dispatch(
            ...
            mouseMove: (params, { moveVector }) => {
                ...
                for (const layerId of layerIds) {
                    ...
                    for (const timelineId of timelineIds) {
                        op.add(shiftTimelineIndex({ timelineId, shiftBy: indexDelta }));
                    }
                }
                op.submit();
            },
    ...
    onMouseDown: (
        ...
        mouseDownMoveAction(e, {
            ...
            mouseUp: (params, hasMoved) => {
                ...
                if (!hasMoved) {
                    params.dispatch(clearCompositionSelection(composition.id));
                    params.dispatch(
                        timelines.map((timeline) => clearTimelineSelection({ timelineId: timeline.id })),
                    );
                ...
                params.dispatch(
                    affectedTimelines.map(({ timelineId, keyframeIds }) => {
                        return addKeyframesToSelection({ timelineId, keyframeIds });
                    }),
                );
