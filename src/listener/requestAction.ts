...
import { appHistorySlice } from "~/state/history/appHistorySlice";
...
const performRequestedAction = (
    ...
    const cancelAction = () => {
        store.dispatch(appHistorySlice.actions.cancelAction({ actionId }));
        onComplete();

        const diffsToPerform = reverseDiffs.length ? reverseDiffs : [...allDiffs].reverse();
        sendDiffsToSubscribers(adaptState(getActionState()), diffsToPerform, "backward");
    };

    store.dispatch(appHistorySlice.actions.startAction({ actionId }));
    ...
    const dispatch: RequestActionParams["dispatch"] = (action, ...args) => {
        ...
        const shouldSkipHistory = (a: Action) => a.meta?.skipHistory === true;
            ...
            if (actions.some(shouldSkipHistory)) {
                ...
                actions.forEach(a => {
                    if (shouldSkipHistory(a)) {
                        ...
                        store.dispatch(appHistorySlice.actions.dispatchToAction({
                            actionId,
                            actionToDispatch: a,
                            modifiesHistory: history
                        }));
                    }
                });
            } else {
                store.dispatch(appHistorySlice.actions.dispatchBatchToAction({
                    actionId,
                    actionBatch: actions,
                    modifiesHistory: history
                }));
    ...
    const params: RequestActionParams = {
        ...
        submitAction: (name = "Unknown action", options = {}) => {
            ...
            for (const shouldAddToStack of shouldAddToStackFns) {
                if (shouldAddToStack(adaptState(getCurrentState()), adaptState(getActionState()))) {
                    addToStack = true;
                }
            }
            ...
            if (diffs.length) {
                sendDiffsToSubscribers(adaptState(getActionState()), diffs);
            }

            store.dispatch(
                appHistorySlice.actions.submitAction({
                    actionId,
                    name,
                    modifiesHistory: history,
                    modifiedKeys: [],
                    allowIndexShift,
                    diffs: [...diffs, ...addedButNotPerformedDiffs]
                })
            );
            onComplete();
        },
        ...
        performDiff: (fn) => {
            ...
            sendDiffsToSubscribers(adaptState(getActionState()), diffsToPerform);
        },
        addReverseDiff: (fn) => {
            const result = fn(diffFactory);
            const diffsToAdd = Array.isArray(result) ? result : [result];
